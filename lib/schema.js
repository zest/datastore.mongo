...
module.exports = specifications.components.DataStoreProvider.extend(
     ...
     */
    {
         ...
         */
        create: function (storeSchema) {
            return this.connection.then(function (db) {
                return q.Promise(function (resolve) {
                    var schema = mongoose.Schema(storeSchema.structure);
                    if (storeSchema.methods) {
                        Object.keys(storeSchema.methods).forEach(function (methodName) {
                            schema[methodName] = storeSchema.methods[methodName];
                        });
                    }
                    if (storeSchema.statics) {
                        Object.keys(storeSchema.statics).forEach(function (staticName) {
                            schema.statics[staticName] = storeSchema.statics[staticName];
                        });
                    }
                    ...
                    resolve(new store(db.model(storeSchema.name, schema, storeSchema.name)));
                });
            });
        },
        /**
         ...
         * @throws Function is un-implemented!
         */
        read: function (schemaName) {
            logger.log('trying to find schema', schemaName);
            return this.connection.then(function (db) {
                logger.log('found schema', schemaName, db.model(schemaName));
                return resolve(new store(db.model(schemaName)));
            });
        },
        /**
         * @function
